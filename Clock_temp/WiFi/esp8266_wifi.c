...
ESP_WIFI_IPD_t xWifiIpd[ESP_MAX_SOCKETS];
 ...
 */
static void ParseApDetail( char * string, ESP_Scan_t *xBuffer )
{
    char *pcDelim = ",";
    char *pcPtr;
    char *pcSave;
    ...
    while (pcPtr != NULL) {
        ...
        case 1:
            ...
            xBuffer->xSecurity = (ESP_Security_t)ucMode;
            break;

        case 2:
            pcSSID = strtok(pcPtr, "\"");
            if (pcSSID) {
                strncpy(&xBuffer->cSSID[0], pcSSID, ESP_MAX_SSID_LEN + 1);
            }
            break;
 ...
 */
static void AT_ParseAccessPoint( ESP_WIFI_Object_t * pxObj, ESP_Scan_t * pxBuffer, uint8_t ucNumNetworks )
{
    char *pcDelim = "()\r\n";
    char *pcPtr;
    char *pcSave;
    ...
    for (ucCount = 0; ucCount < ucNumNetworks; ) {
        if (pcPtr != NULL){
            if (strcmp(pcPtr, "+CWLAP:") == 0) {
                pcPtr = strtok_r(NULL, pcDelim, &pcSave);
                ParseApDetail(pcPtr, (ESP_Scan_t *)(&pxBuffer[ucCount]));
                ucCount++;
            }
            pcPtr = strtok_r(NULL, pcDelim, &pcSave);
        } else
 ...
 */
BaseType_t ESP_IO_Init( ESP_WIFI_Object_t * pxObj )
{
  ...
  HAL_UART_Receive_IT(&WiFiUartHandle, (uint8_t *)&rx_buf, 1);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *UartHandle)
{
  ...
  HAL_UART_Receive_IT(UartHandle, (uint8_t *)&rx_buf[rx_buf_widx], 1);
}
                               ...
                               uint8_t ucLinkID, TickType_t xTimeout )
{
            ...
            if (xTaskGetTickCount() > xTickTimeout) {
                ...
                if ((strstr((char *)pucRxBuf, "\r\nRecv") > 0) && (strstr((char *)pucRxBuf, "bytes\r\n") > 0)) {
                    ...
                    if (xTimeout > pdMS_TO_TICKS(ESP_WIFI_NONBLOCK_RECV_TO)) {
                        ESP_PRINTF(("ERROR: [%s] usReadBytes %d reach the timeout %d !!\n", __func__, usReadBytes, xTimeout));
                    }
                    ucExit = 1;
                    break;
                }
    ...
    if (usCount < 100) {
        if (usCount > 0)
            ESP_PRINTF(("[%s] \"%s\", usCount = %d, xRet = %d\n", __func__, pucRxBuf, usCount, xRet));
        else {
            ESP_PRINTF(("."));
...
ESP_WIFI_Status_t ESP_WIFI_Reset( ESP_WIFI_Object_t * pxObj )
{
    ...
    RX_BUF_RESET();
    for (ucCount = 0; ucCount < ESP_MAX_SOCKETS; ucCount++) {
...
ESP_WIFI_Status_t ESP_WIFI_StartClient( ESP_WIFI_Object_t * pxObj, ESP_WIFI_Conn_t * pxConn )
{
    ...
    if (xRet != ESP_WIFI_STATUS_OK) {
        ...
        if ((xRet == ESP_WIFI_STATUS_ERROR) && (!strstr((char *)pxObj->CmdData, "ALREADY CONNECTED"))) {
            do {
                ESP_PRINTF(("Socket connect error, Wait for socket close.\n"));
